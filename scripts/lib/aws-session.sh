...
VIGIL_PUBLISH_CRED_MODE=""

load_publish_credentials() {
  local creds_file="${VIGIL_PUBLISH_CREDS_FILE:-${_PUBLISH_REPO_ROOT}/upload-cfn.credentials.sh}"
  ...
  if [ -f "${creds_file}" ]; then
    ...
    VIGIL_PUBLISH_CRED_MODE="static"
    return 0
  fi

  VIGIL_PUBLISH_CRED_MODE="profile"
}

aws_cli() {
  if [ "${VIGIL_PUBLISH_CRED_MODE}" = "static" ]; then
    aws "$@"
    return
  fi
