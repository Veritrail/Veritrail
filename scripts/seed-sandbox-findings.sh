#!/usr/bin/env bash
# Seed a throwaway AWS account with common misconfigurations for Vigil QA.
# Requires: AWS CLI, a sandbox account, and Vigil running locally (docker compose up).
set -euo pipefail
...
cat <<'EOF'
Vigil sandbox QA — minimal seed checklist
=========================================

1. Use a dedicated throwaway AWS account (not prod).
2. Deploy the read-only CFN role from Vigil Accounts → Connect AWS.
...
4. Optional misconfigurations to exercise findings (pick any subset):
   ...
   aws s3api create-bucket --bucket "vigil-qa-$(aws sts get-caller-identity --query Account --output text)-unencrypted" \
   ...
   aws iam create-user --user-name vigil-qa-no-mfa 2>/dev/null || true
   aws iam create-login-profile --user-name vigil-qa-no-mfa --password 'TempPass123!Change' --no-password-reset-required 2>/dev/null || true

5. Re-scan from Findings → Scan and confirm findings appear.
6. Tear down: delete seeded resources and remove the account in Vigil when done.
