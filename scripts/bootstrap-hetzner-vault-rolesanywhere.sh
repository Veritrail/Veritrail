...
#
# What this does:
#   1. Installs Vault OSS, AWS CLI v2, jq, and the AWS IAM Roles Anywhere credential helper.
#   2. Starts a local-only Vault service on 127.0.0.1:8200 using file storage.
#   3. Initializes/unseals Vault if needed and configures a PKI root CA.
#   4. Issues a client certificate for this VPS.
#   5. Optionally creates AWS IAM Roles Anywhere resources in your AWS account.
#   6. Writes an AWS CLI profile using credential_process.
#   7. Optionally updates the Veritrail env file with AWS_PROFILE and TRUST_PRINCIPAL_ARN.
#
# No HashiCorp Cloud/signup is required. This uses Vault OSS from HashiCorp's apt repo.
#
# Usage, full setup from a VPS that already has temporary AWS admin/bootstrap creds:
#   sudo AWS_REGION=eu-west-1 ./scripts/bootstrap-hetzner-vault-rolesanywhere.sh
#
# Usage, local Vault/cert/helper only, no AWS changes:
#   sudo ./scripts/bootstrap-hetzner-vault-rolesanywhere.sh --skip-aws
#
# Security note:
#   The private CA root token and unseal key are written to /root/veritrail-vault-init.json.
#   Back it up securely, then remove it from the host if you have another recovery path.
...
# Narrow this once customer role naming is finalized, for example:
#   arn:aws:iam::*:role/VeritrailCustomerScannerRole-*
...
Important environment variables:
  ...
  AWS_ACCOUNT_ID             Optional. Auto-detected with aws sts get-caller-identity when AWS creds exist.
  ENV_FILE                   Default: .env
  AWS_PROFILE_NAME           Default: veritrail-ra
  RA_ROLE_NAME               Default: VeritrailHetznerControlPlaneRole
  ASSUMABLE_ROLE_RESOURCE    Default: *  (tighten this in production)
  CERT_CN                    Default: veritrail-hetzner-control-plane
  CERT_TTL                   Default: 720h

AWS bootstrap credentials:
  For the AWS resource creation step, the command needs temporary AWS credentials with permissions to create:
    - iam role / inline policy
    - rolesanywhere trust anchor / profile
...
require_root() {
  if [[ "$(id -u)" -ne 0 ]]; then
    die "Run with sudo/root so Vault, systemd, and /etc/veritrail can be configured"
  fi
}

install_base_packages() {
  command -v apt-get >/dev/null 2>&1 || die "This bootstrap supports Ubuntu/Debian hosts with apt-get"
...
install_vault() {
  if command -v vault >/dev/null 2>&1; then
    log "Vault CLI already installed: $(vault version | head -1)"
    return 0
  fi

  log "Installing HashiCorp Vault OSS from HashiCorp apt repo..."
...
install_aws_cli_v2() {
  ...
  case "$arch" in
    ...
    *) die "Unsupported architecture for AWS CLI v2: $arch" ;;
  esac
  tmpdir="$(mktemp -d)"
...
install_rolesanywhere_helper() {
  if command -v aws_signing_helper >/dev/null 2>&1; then
    log "aws_signing_helper already installed: $(aws_signing_helper version 2>/dev/null || true)"
    return 0
  fi
  ...
  case "$arch" in
    ...
    *) die "Unsupported architecture for aws_signing_helper: $arch" ;;
  esac
...
init_and_unseal_vault() {
  if ! vault_is_initialized; then
    log "Initializing Vault with one unseal key..."
  ...
  [[ -f "$VAULT_INIT_FILE" ]] || die "Vault is initialized, but $VAULT_INIT_FILE is missing. Export VAULT_TOKEN and unseal manually, then re-run."
...
configure_pki() {
  log "Configuring Vault PKI at path $VAULT_PKI_PATH..."
...
issue_client_certificate() {
  if [[ "$FORCE_CERT" -eq 0 && -s "$STATE_DIR/client.pem" && -s "$STATE_DIR/client.key" ]]; then
    log "Client certificate already exists — skipping issue (use --force-cert to rotate)"
    return 0
  fi

  log "Issuing client certificate from Vault PKI..."
  local issue_json
  issue_json="$(mktemp)"
  vault write -format=json "$VAULT_PKI_PATH/issue/$VAULT_PKI_ROLE" \
    common_name="$CERT_CN.$CERT_DOMAIN" \
    ttl="$CERT_TTL" >"$issue_json"
...
aws_identity_account_id() {
  aws sts get-caller-identity --query Account --output text 2>/dev/null || true
}

ensure_aws_account_id() {
  if [[ -z "$AWS_ACCOUNT_ID" ]]; then
    AWS_ACCOUNT_ID="$(aws_identity_account_id)"
  fi
  [[ -n "$AWS_ACCOUNT_ID" && "$AWS_ACCOUNT_ID" != "None" ]] || die "AWS_ACCOUNT_ID not set and aws sts get-caller-identity failed. Export AWS bootstrap creds or set AWS_ACCOUNT_ID."
}
...
ensure_trust_anchor() {
  local existing arn input ca_data
...
ensure_iam_role() {
  ...
  if aws iam get-role --role-name "$RA_ROLE_NAME" >/dev/null 2>&1; then
    log "IAM role exists — updating trust policy: $RA_ROLE_NAME"
  ...
  aws iam put-role-policy \
    ...
    --policy-name VeritrailAssumeCustomerRoles \
...
ensure_rolesanywhere_profile() {
  ...
  profile_arn="$(aws rolesanywhere create-profile \
    --region "$AWS_REGION" \
    --name "$RA_PROFILE_NAME" \
    --role-arns "$role_arn" \
    --duration-seconds "$RA_SESSION_DURATION" \
    --enabled \
...
write_aws_profile() {
  ...
  log "Writing AWS profile [$AWS_PROFILE_NAME] to $config_file"
...
cp.read(config_file)
if not cp.has_section(section) and section != "default":
    cp.add_section(section)
elif section == "default" and not cp.has_section(section):
    cp.add_section(section)
credential_process = (
...
update_veritrail_env() {
  ...
  if [[ ! -f "$env_path" ]]; then
    if [[ -f "$REPO_DIR/.env.example" ]]; then
      ...
      warn "No $ENV_FILE or .env.example found — skipping env update"
      return 0
    fi
  fi
...
test_rolesanywhere_profile() {
  local target_user="${SUDO_USER:-root}"
  log "Testing AWS profile with sts get-caller-identity..."
  if [[ "$target_user" == "root" ]]; then
    AWS_PROFILE="$AWS_PROFILE_NAME" aws sts get-caller-identity || warn "AWS profile test failed"
  else
    sudo -u "$target_user" AWS_PROFILE="$AWS_PROFILE_NAME" aws sts get-caller-identity || warn "AWS profile test failed"
  fi
}
...
Recommended next hardening:
  1. Set ASSUMABLE_ROLE_RESOURCE to the exact customer role ARN pattern, not '*'.
  2. Back up $VAULT_INIT_FILE securely, then remove it from the VPS if you have another recovery path.
  3. Rotate/re-issue the client certificate periodically with --force-cert.
  4. Consider moving Vault unseal keys/token to an offline password manager.
================================================================================
EOF
}
...
main() {
  ...
  if [[ "$SKIP_AWS" -eq 1 ]]; then
    warn "Skipping AWS Roles Anywhere creation. Run again without --skip-aws after AWS bootstrap creds are available."
    print_done
    return 0
  fi
