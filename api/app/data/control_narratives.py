...
Each narrative describes what the control covers and what evidence Veritrail collects.
"""
...
NARRATIVES: dict[str, str] = {
    ...
    # ── CIS AWS L1 ──────────────────────────────────────────────────────────
    "CIS 1.4": (
        "Veritrail verifies that the AWS root account does not have active access keys. "
        "Evidence is collected from the IAM credential report at each scan."
    ),
    "CIS 1.5": (
        "Veritrail verifies that MFA is enabled on the AWS root account. "
        "Evidence is collected from the IAM account summary at each scan."
    ),
    "CIS 1.10": (
        "Veritrail enumerates all IAM users with console access and verifies MFA device enrollment. "
        "Users with no MFA device are reported as findings. Evidence is collected from IAM at each scan."
    ),
    "CIS 1.14": (
        "Veritrail verifies that IAM users do not have access keys older than 90 days without rotation. "
        "Key creation date and last-used date are collected from IAM at each scan."
    ),
    "CIS 1.16": (
        "Veritrail verifies that IAM policies are not attached directly to users. "
    ...
    "CIS 1.7": (
        "Veritrail checks CloudTrail for root account API activity in the last 90 days. "
        "Any root usage is reported as a finding — routine operations should use IAM roles or users."
    ),
    "CIS 1.8": (
        "Veritrail reads the account IAM password policy and flags weak minimum length or missing "
        "complexity requirements per CIS guidance. Policy fields are collected at each scan."
    ),
    "CIS 1.9": (
        "Veritrail verifies password reuse prevention is enabled on the IAM account password policy "
        "(password reuse prevention / history). Evidence is collected from IAM at each scan."
    ),
    "CIS 1.12": (
        "Veritrail flags IAM users inactive for 90+ days and access keys with no recorded usage "
        "in the same window, supporting credential disablement per CIS guidance."
    ),
    "CIS 1.22": (
        "Veritrail scans customer-managed role policies for the CIS full-admin pattern: "
        "Allow with Action: '*' and Resource: '*' on the same statement."
    ),
    "CIS 1.20": (
        "Veritrail verifies that a support role exists in the account for incident management. "
        "IAM roles with AWSSupportAccess policy attachment are checked."
    ),
    "CIS 1.1": (
        "Veritrail verifies the AWS account primary contact details are complete so billing and "
    ...
    "CIS 1.2": (
        "Veritrail verifies an alternate security contact is registered on the AWS account so AWS "
    ...
    "CIS 1.3": (
        "Veritrail verifies the AWS root account has no programmatic access keys. Root credential "
        "state is read from the IAM credential report at each scan; any root access key is "
        "reported as a finding."
    ),
    "CIS 1.6": (
        "Veritrail checks CloudTrail for root user activity. Root sign-in and API events are flagged "
        "so routine administration is performed with scoped IAM roles or users rather than root."
    ),
    "CIS 1.11": (
        "Veritrail flags IAM users with no console sign-in and access keys with no recorded API use "
        "within the CIS 45-day threshold, supporting credential deactivation per benchmark "
        "guidance. Detection is automated on each scan; Veritrail never disables or "
        "deletes credentials — your team performs that change in AWS using the remediation "
        "guidance on each finding."
    ),
    "CIS 1.13": (
        "Veritrail flags IAM access keys older than the rotation threshold without rotation. Key "
        "creation and last-used dates are collected from the IAM credential report at each scan."
    ),
    "CIS 1.15": (
        "Veritrail scans customer-managed IAM policies for the full-administrative pattern "
    ...
    "CIS 1.17": (
        ...
        "long-lived IAM user access keys. Veritrail does not automate detection of static credentials "
        "on instances, so this control requires manual attestation; related Veritrail evidence "
        "includes the IAM access-key inventory and unused-key findings."
    ),
    "CIS 1.18": (
        "Veritrail enumerates IAM server certificates and flags any past their expiration date. "
        "Certificate metadata is collected via iam:ListServerCertificates at each scan."
    ),
    "CIS 1.21": (
        "Veritrail checks whether the AWSCloudShellFullAccess managed policy is attached to IAM "
    ...
    "CIS 2.1.4": (
        "Veritrail verifies S3 Block Public Access is enabled at both the account level and per "
        "bucket. Account and bucket public-access configuration is collected at each scan; "
        "any gap is reported as a finding."
    ),
    "CIS 2.1": (
        "Veritrail verifies that AWS CloudTrail is enabled and covers all regions. "
        "Trail configuration is collected via cloudtrail:DescribeTrails at each scan."
    ),
    "CIS 2.2": (
        "Veritrail verifies that CloudTrail log file validation is enabled on all trails. "
        "This ensures log integrity for audit sampling across the evidence period."
    ),
    "CIS 2.4": (
        "Veritrail verifies that CloudTrail trails are integrated with CloudWatch Logs. "
        "Trail configuration is collected at each scan."
    ),
    "CIS 2.6": (
        "Veritrail verifies that S3 bucket access logging is enabled on the CloudTrail delivery bucket. "
        "S3 bucket logging configuration is collected at each scan."
    ),
    "CIS 3.1": (
        "Veritrail collects CloudTrail events for root account activity. "
        "Any use of root credentials triggers a finding flagged to CC6 and this control."
    ),
    "CIS 2.1.1": (
        "Veritrail verifies that S3 bucket policies deny HTTP requests using an aws:SecureTransport condition."
    ),
    "CIS 2.1.2": (
        "Veritrail verifies that S3 buckets have default encryption (SSE-S3 or SSE-KMS) enabled."
    ),
    "CIS 2.1.3": (
        "Veritrail checks whether S3 buckets have MFA Delete enabled on versioning configuration."
    ),
    "CIS 3.1": (
        "Veritrail verifies that at least one CloudTrail trail is enabled and logging management events."
    ),
    "CIS 3.2": (
        "Veritrail verifies CloudTrail log file validation is enabled on collected trails."
    ),
    "CIS 3.5": (
        "Veritrail verifies AWS Config recorder status per region."
    ),
    "CIS EC2.2": (
        "Veritrail flags VPC default security groups that contain inbound or outbound rules."
    ),
    "CIS 4.3": (
        "Veritrail verifies VPC flow logging is enabled for collected VPCs."
    ),
    "CIS 4.4": (
        "Veritrail verifies EC2 instances require IMDSv2 (HttpTokens required)."
    ),
    "CIS 5.1": (
        "Veritrail verifies EBS encryption-by-default is enabled per region."
    ),
    "CIS 1.19": (
        "Veritrail verifies IAM Access Analyzer is enabled in scanned regions."
    ),
    "CIS 1.16": (
        "Veritrail verifies at least one IAM role can access AWS Support via AWSSupportAccess."
    ),
    "CIS 3.3": (
        "Veritrail verifies CloudTrail trails are configured to use KMS encryption for log delivery."
    ),
    "CIS 2.1.5": (
        "Veritrail verifies account-level and per-bucket S3 Block Public Access settings."
    ),
    "CIS 2.2.1": (
        "Veritrail enumerates EBS volumes and flags any that are not encrypted at rest."
    ),
    "CIS 2.3.1": (
        "Veritrail verifies RDS instance storage encryption is enabled."
    ),
    "CIS 2.3.2": (
        "Veritrail flags RDS instances that are publicly accessible."
    ),
    "CIS 2.3.3": (
        "Veritrail verifies RDS automated backup retention is greater than zero."
    ),
    "CIS 3.3": (
        "Veritrail verifies CloudTrail log delivery S3 buckets are not publicly accessible."
    ),
    "CIS 3.4": (
        "Veritrail verifies CloudTrail trails send logs to CloudWatch Logs."
    ),
    "CIS 3.6": (
        "Veritrail verifies server access logging is enabled on CloudTrail delivery buckets."
    ),
    "CIS 3.8": (
        "Veritrail verifies customer-managed KMS keys have automatic rotation enabled. "
        "CloudTrail event analysis also flags attempts to disable GuardDuty detectors."
    ),
    "CIS 3.9": (
        "Veritrail monitors CloudTrail for StopConfigurationRecorder and related calls that "
        "may indicate an attempt to disable AWS Config compliance recording."
    ),
    "CIS 3.x": (
        "Veritrail analyzes CloudTrail management events for logging and key-management tampering, "
        "including trail stop/delete/update, KMS key disable, and scheduled key deletion."
    ),
    "CIS 4.1": (
        "Veritrail flags security groups allowing SSH (port 22) or RDP (port 3389) from the internet. "
        "CloudTrail event analysis detects weakening or removal of S3 Block Public Access settings."
    ),
    "CIS 4.x": (
        "Veritrail monitors CloudTrail for S3 bucket policy and ACL changes that could expose data "
        "to unintended principals."
    ),
    "CIS 5.x": (
        "Veritrail monitors CloudTrail for security group ingress changes that open sensitive ports "
        "to 0.0.0.0/0 or ::/0."
    ),
    "CIS 4.2": (
        "Veritrail flags security groups allowing RDP (port 3389) from 0.0.0.0/0 or ::/0."
    ),

    # ── ISO 27001 ────────────────────────────────────────────────────────────
    "A.9.2.1": (
        "Veritrail provides evidence of user registration and de-registration through IAM user inventory, "
        "dormancy checks, and access key lifecycle tracking. "
        "GitHub and GitLab member rosters are collected where integrated."
    ),
    "A.9.2.2": (
        "Veritrail verifies that access provisioning follows least-privilege by checking role permission usage "
        "against actual service calls (via AWS access advisor) and flagging excessive grants."
    ),
    "A.9.2.3": (
        "Veritrail collects evidence of privileged account controls: root MFA, no root access keys, "
        "wildcard IAM action grants, and admin-scope roles not assumed in 90+ days."
    ),
    "A.9.2.4": (
        "Veritrail verifies that secret authentication information (access keys) is rotated within 90 days "
        "and that MFA is enforced for console users."
    ),
    "A.9.2.5": (
        "Veritrail supports periodic access reviews by collecting IAM user/role/key inventory with "
        "last-activity timestamps and generating CSV exports of the access state at each scan."
    ),
    "A.9.2.6": (
        "Veritrail flags access keys and role assumptions inactive for 90+ days, supporting "
        "timely revocation of access rights for leavers and role cleanup."
    ),
    "A.9.4.1": (
        "Veritrail restricts and monitors access to information assets by flagging S3 public exposure "
        "risks and analyzing CloudTrail for bucket policy and public-access-block changes."
    ),
    "A.9.4.2": (
        "Veritrail verifies MFA enrollment for all console-access IAM users and GitHub/GitLab organization members."
    ),
    "A.10.1.1": (
        "Veritrail verifies encryption at rest: S3 bucket default encryption, KMS key rotation status, "
        "RDS and EBS storage encryption, and related cryptographic control configuration."
    ),
    "A.10.1.2": (
        "Veritrail verifies that key management policies are in place: KMS key rotation enabled, "
        "CloudTrail delivery encrypted, S3 buckets using SSE-KMS where required."
    ),
    "A.12.1.2": (
        "Veritrail supports change management by collecting CloudTrail evidence of Lambda function "
        "creation and configuration updates alongside other infrastructure mutations."
    ),
    "A.12.4.1": (
        "Veritrail verifies that event logging is active: CloudTrail enabled with all-regions coverage, "
        "log file validation enabled, VPC flow logs enabled, S3 server access logging enabled."
    ),
    "A.12.6.1": (
        "Veritrail verifies that technical vulnerability controls are operating: "
    ...
    "A.13.1.1": (
        "Veritrail collects evidence of network security controls: "
    ...
    "A.13.1.3": (
        "Veritrail verifies network segregation controls through VPC flow log collection, "
        "security group rule inventory, and RDS public accessibility checks."
    ),
    "A.12.4.2": (
        "Veritrail verifies log integrity and protection: CloudTrail log file validation, "
        "KMS encryption on trails, and that delivery buckets are not public."
    ),
    "A.13.2.3": (
        "Veritrail verifies data-in-transit controls: S3 HTTPS-only bucket policies and "
...
PLATFORM_SCOPE_LIMITATIONS: dict[str, list[str]] = {
    "soc2": [
        ...
        "and is not directly monitored by Veritrail. Obtain physical and environmental control evidence "
        "from your provider, colo vendor, or SOC 2 report.",
        "Personnel and HR processes (hiring, background checks, termination) are outside Veritrail scope "
    ...
    "cis_aws_l1": [
        ...
        "responsibility; Veritrail does not assess provider datacenter security.",
    ],
    "iso27001": [
        "Physical security controls (ISO A.11) for cloud-hosted workloads rely on provider attestation; "
        "Veritrail does not monitor datacenter physical access.",
    ],
}

# Control-specific scope limits — only shown on the relevant control detail page.
KNOWN_GAPS: dict[str, list[str]] = {
    "CC6.2": ["HR/offboarding attestations for non-AWS identities are outside Veritrail scope."],
    ...
    "CIS 1.16": ["Veritrail automates policy attachment checks; manual attestation still needed for business justification."],
}
...
def evidence_refs_from_checks(check_ids: list[str]) -> list[str]:
    if not check_ids:
        return ["No automated Veritrail checks mapped — manual attestation required."]
    refs: list[str] = []
...
_READ_ONLY_POSTURE = (
    "Veritrail performs detection only — it never disables, deletes, rotates, "
    ...
    "Veritrail surfaces per-finding console/CLI guidance and re-verifies on the next scan."
)
...
def scope_limitations_for(framework: str) -> list[str]:
    ...
    README, source_manifest.json, and PDF all state that Veritrail never writes to customer AWS.
    """
...
def narrative_detail_for(
    ...
    if not check_ids:
        gaps.insert(0, "No automated checks mapped — this control requires manual attestation in Veritrail.")
    return {
