...
class VigilEvidencePDF(FPDF):
    ...
    def footer(self) -> None:
        ...
        self.cell(
            0,
            3.8,
            _s(f"Generated by Vigil  |  {self.framework_short}  |  Last {self.period_days} days  |  Report ID {self.report_id}"),
            align="C",
        )
...
def _draw_evidence_sources(pdf: FPDF, sources: list[str], framework: str, scope_limitations: list[str]) -> None:
    ...
    if not scope_limitations:
        scope_limitations = [
            "Vigil performs detection only and never modifies any resource.",
