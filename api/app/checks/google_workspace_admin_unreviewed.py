...
CHECK_ID = "google_workspace.admin.unreviewed"
    ...
    for provider in _providers_of_type(db, account_id, "google_workspace"):
        ...
        for u in admins:
            ...
            out.append(
                FindingDraft(
                    check_id=CHECK_ID,
                    resource_arn=f"google_workspace://{source}/{u.external_id}",
                    title=f"Google Workspace admin `{u.email or u.external_id}` requires access review",
                    severity="high",
