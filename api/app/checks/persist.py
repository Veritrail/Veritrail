...
def persist_scope_findings(
    ...
    *,
    ...
    """Persist findings scoped to a GCP project or Azure subscription."""
    if scope_column not in {"gcp_project_id", "azure_subscription_id"}:
        raise ValueError(f"unsupported scope column: {scope_column}")

    scope_col = getattr(Finding, scope_column)
    now = datetime.now(timezone.utc)
    opened = 0
    by_key = {(d.check_id, d.resource_arn): d for d in drafts}
