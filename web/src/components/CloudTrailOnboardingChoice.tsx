  ...
  return (
    <div className="rounded-lg border border-sky-100 bg-sky-50/60 px-4 py-3 text-sm text-zinc-700">
      ...
      <p className="mt-1 text-xs leading-relaxed text-zinc-600">
        Vigil never auto-provisions trails silently. Choose whether this account already has an org or local trail, or
        whether you plan to deploy a Vigil-managed trail via remediation.
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button
          type="button"
          disabled={isLoading || save.isPending}
          onClick={() => save.mutate("vigil_managed")}
          className={`rounded-md px-3 py-1.5 text-xs font-semibold ring-1 ${
            mode === "vigil_managed"
              ? "bg-white text-sky-800 ring-sky-300"
              : "bg-white/70 text-zinc-700 ring-zinc-200 hover:ring-zinc-300"
