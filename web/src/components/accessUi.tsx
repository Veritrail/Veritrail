...
export const accessPrimaryBtn =
  "h-10 shrink-0 rounded-lg border border-zinc-200 bg-white px-5 text-sm font-semibold text-zinc-800 shadow-sm transition hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-50";
...
export const accessComposer = "rounded-xl border border-zinc-200 bg-zinc-50/60 p-3";
...
export function Avatar({ email, name }: { email: string; name?: string | null }) {
  ...
  return (
    <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-semibold ${tone}`}>
      {initial}
    </div>
  );
}
...
export function AccessRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-zinc-100 bg-white px-3 py-2.5 transition hover:border-zinc-200 hover:bg-zinc-50/60">
      <Avatar email={email} />
