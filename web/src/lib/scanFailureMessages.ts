...
export function friendlyScanFailureMessage(raw: string): string {
  ...
  if (lower.includes("throttl") || lower.includes("rate exceeded") || lower.includes("too many requests")) {
    return "AWS temporarily limited how fast Vigil could scan. Wait a few minutes and run Re-scan on Findings.";
  }
  ...
  if (
    ...
    return (
      ...
      "Open Accounts to verify your connector, then run Re-scan on Findings. " +
  ...
  if (lower.includes("error") || lower.includes("exception") || lower.includes("failed")) {
    return (
      ...
      "Verify your AWS connection on Accounts, then run Re-scan on Findings."
    );
  }
