 ...
 * Plain, factual: what data Veritrail reads and what condition triggers the finding.
 */
  ...
  switch (checkId) {
    case "iam.access_key.unused_90d":
      return copy(
        ...
        "Veritrail reads IAM GetAccessKeyLastUsed for each active key. Keys with no recorded API use within the 90-day lookback are flagged as unused.",
      );
    case "iam.access_key.unused_45d":
      return copy(
        ...
        "Veritrail reads IAM GetAccessKeyLastUsed for each active key. Keys with no recorded API use within the 45-day lookback are flagged as unused.",
      );
    case "iam.access_key.no_rotation_90d":
      return copy(
        ...
        "Veritrail reads each access key's create date from IAM. Keys older than the configured rotation-age threshold are flagged.",
      );
    case "iam.access_key.multiple_active":
      return copy(
        ...
        "Veritrail lists active access keys per IAM user. Users with more than one key in Active status are flagged.",
      );
    default:
      return copy(
        "Verify that IAM programmatic credentials meet your access-key hygiene policy.",
        "Veritrail evaluates IAM access keys using AWS last-used timestamps and key status on each scan.",
      );
  }
}
  ...
  switch (checkId) {
    case "iam.user.no_mfa":
      return copy(
        ...
        "Veritrail lists IAM users with console password enabled and checks for registered MFA devices. Users without MFA are flagged.",
      );
    case "iam.user.inactive_90d":
      return copy(
        ...
        "Veritrail reads sign-in and API last-activity timestamps per IAM user. Users with no activity within the inactivity window are flagged.",
      );
    case "iam.user.credentials_unused_45d":
      return copy(
        ...
        "Veritrail reads console password last-used per IAM user. Users with no sign-in within the 45-day lookback are flagged.",
      );
    case "iam.user.direct_policy_attachment":
      return copy(
        ...
        "Veritrail lists IAM user policy attachments. Users with customer-managed or inline policies attached directly (not via groups) are flagged.",
      );
    default:
      return copy(
        "Verify that IAM user identity controls meet your baseline policy.",
        "Veritrail evaluates IAM user MFA, activity, and policy attachment posture on each scan.",
      );
  }
}
  ...
  switch (checkId) {
    case "iam.role.unassumed_90d":
      return copy(
        ...
        "Veritrail reads role last-used data from IAM. Roles with no AssumeRole use within the lookback period are flagged.",
      );
    case "iam.role.least_privilege_policy":
      return copy(
        ...
        "Veritrail flags customer-managed inline or attached policies with Action \"*\". Resource \"*\" is full admin; scoped resources are wildcard-action violations.",
      );
    case "iam.role.unused_services_90d":
      return copy(
        ...
        "Veritrail compares each role's granted IAM services against service last-accessed data. Services with no recorded use in the lookback window are flagged.",
      );
    case "iam.role.trust_wildcard":
      return copy(
        "Verify that IAM role trust policies do not allow any principal (\"*\") to attempt AssumeRole.",
        "Veritrail parses role trust policies. Trust allowing Principal \"*\" for sts:AssumeRole is flagged.",
      );
    case "iam.role.external_account_trust":
      return copy(
        ...
        "Veritrail parses role trust policies for sts:AssumeRole. Grants to principals outside the scanned account are flagged.",
      );
    default:
      return copy(
        "Verify that IAM role trust and permission scope meet your baseline policy.",
        "Veritrail evaluates IAM role trust policies and attached permission scope on each scan.",
      );
  }
}
  ...
  switch (checkId) {
    case "iam.policy.wildcard_resource":
      return copy(
        ...
        "Veritrail parses customer-managed and inline policy statements. Write-capable statements with Resource \"*\" are flagged.",
      );
    case "iam.policy.unattached":
      return copy(
        ...
        "Veritrail lists customer-managed policies and attachment targets. Policies with no principals attached are flagged when this optional check is enabled.",
      );
    default:
      return copy(
        "Verify that customer-managed IAM policies meet attachment and scope expectations.",
        "Veritrail lists customer-managed policies and evaluates attachments and statement scope on each scan.",
      );
  }
}
  ...
  switch (checkId) {
    case "iam.root.no_mfa":
      return copy(
        ...
        "Veritrail reads root MFA configuration from the account summary. Root without a registered MFA device is flagged.",
      );
    case "iam.root.has_access_keys":
      return copy(
        "Verify that the AWS root account has no active access keys.",
        "Veritrail reads root access key status from the account summary. Any active root access key is flagged.",
      );
    case "iam.root.usage":
      return copy(
        "Verify that the AWS root account is not used for routine API operations.",
        "Veritrail reads credential-report and CloudTrail data for recent root API activity. Recent root API use is flagged.",
      );
    default:
      return copy(
        "Verify that AWS root account credential and usage controls meet baseline expectations.",
        "Veritrail reads root credential configuration and recent root API activity on each scan.",
      );
  }
}
...
function s3(checkId: string, _s: RemediationSummary): CheckComplianceCopy {
  ...
  if (checkId.includes("public_access")) {
    return copy(
      ...
      `Veritrail reads S3 Block Public Access settings at ${checkId.startsWith("s3.account") ? "account" : "bucket"} scope. Any of the four block settings disabled is flagged.`,
    );
  }
  if (checkId === "s3.bucket.no_https_policy") {
    return copy(
      "Verify that S3 bucket policies deny unencrypted HTTP access (aws:SecureTransport).",
      "Veritrail parses bucket policies for a Deny on aws:SecureTransport false. Buckets without that statement are flagged.",
    );
  }
  if (checkId === "s3.bucket.no_kms" || checkId === "s3.bucket.no_default_encryption") {
    return copy(
      ...
      "Veritrail reads bucket default encryption configuration. Buckets without default encryption at rest are flagged.",
    );
  }
  if (checkId === "s3.bucket.no_logging") {
    return copy(
      "Verify that S3 server access logging is enabled on data buckets.",
      "Veritrail reads bucket logging configuration. Buckets without server access logging enabled are flagged.",
    );
  }
  if (checkId === "s3.bucket.no_mfa_delete") {
    return copy(
      "Verify that versioned S3 buckets have MFA Delete enabled where required.",
      "Veritrail reads versioning and MFA Delete settings. Versioned buckets without MFA Delete enabled are flagged.",
    );
  }
  return copy(
    `Verify that S3 ${bucket} configuration meets your baseline policy.`,
    "Veritrail reads S3 encryption, public access, and logging configuration per bucket on each scan.",
  );
}
  ...
  switch (checkId) {
    case "cloudtrail.trail.not_enabled":
      return copy(
        ...
        "Veritrail lists CloudTrail trails per region. Regions without an active trail recording management events are flagged.",
      );
    case "cloudtrail.trail.no_log_validation":
      return copy(
        "Verify that CloudTrail log file validation is enabled on production trails.",
        "Veritrail reads trail log file validation settings. Trails without validation enabled are flagged.",
      );
    case "cloudtrail.trail.no_kms":
      return copy(
        "Verify that CloudTrail trails encrypt log files with a customer-managed KMS key.",
        "Veritrail reads trail KMS encryption settings. Trails without KMS log encryption are flagged.",
      );
    case "cloudtrail.trail.s3_bucket_public":
      return copy(
        "Verify that the CloudTrail S3 log bucket is not publicly accessible.",
        "Veritrail reads Block Public Access and bucket policy on the trail S3 bucket. Publicly accessible trail buckets are flagged.",
      );
    case "cloudtrail.trail.no_cloudwatch_logs":
      return copy(
        ...
        "Veritrail reads CloudWatch Logs integration on trails. Trails without CloudWatch delivery are flagged.",
      );
    case "cloudtrail.trail.s3_bucket_no_logging":
      return copy(
        "Verify that server access logging is enabled on the CloudTrail S3 log bucket.",
        "Veritrail reads S3 access logging on the trail log bucket. Trail buckets without access logging are flagged.",
      );
    default:
      return copy(
        "Verify that CloudTrail trail coverage and log integrity settings meet baseline expectations.",
        "Veritrail evaluates CloudTrail trail coverage and log integrity settings per region on each scan.",
      );
  }
}
  ...
  switch (checkId) {
    case "ec2.security_group.unrestricted_ssh":
      return copy(
        ...
        "Veritrail reads security group ingress rules. Rules allowing tcp/22 from 0.0.0.0/0 or ::/0 are flagged.",
      );
    case "ec2.security_group.unrestricted_rdp":
      return copy(
        "Verify that security groups do not expose RDP (tcp/3389) to the entire internet.",
        "Veritrail reads security group ingress rules. Rules allowing tcp/3389 from 0.0.0.0/0 or ::/0 are flagged.",
      );
    case "ec2.security_group.default_allows_traffic":
      return copy(
        ...
        "Veritrail reads the VPC default security group. Non-empty inbound or outbound rule sets are flagged.",
      );
    default:
      return copy(
        "Verify that EC2 security group ingress rules meet your network baseline.",
        "Veritrail evaluates security group ingress against sensitive port baselines on each scan.",
      );
  }
}
...
function github(checkId: string, _s: RemediationSummary): CheckComplianceCopy {
  if (checkId === "github.repo.no_branch_protection") {
    return copy(
      ...
      "Veritrail reads branch protection rules on the default branch via the GitHub API. Repositories without required pull-request reviews or status checks are flagged.",
    );
  }
  if (checkId === "github.repo.no_env_protection") {
    return copy(
      ...
      "Veritrail reads GitHub environment protection rules. Production-like environments without required reviewers are flagged; non-production environments are reported at Medium severity.",
    );
  }
  if (checkId === "github.repo.no_codeowners" || checkId === "gitlab.repo.no_codeowners") {
    return copy(
      ...
      "Veritrail reads repository CODEOWNERS file presence. Repositories without CODEOWNERS are flagged when this optional check is enabled.",
    );
  }
  if (checkId.startsWith("github.org.")) {
    return copy(
      "Verify that GitHub organization security settings meet your identity baseline.",
      "Veritrail syncs GitHub organization security settings via the GitHub API and flags values outside the configured baseline.",
    );
  }
  return copy(
    "Verify that GitHub repository branch protection and required review settings meet your change-management baseline.",
    "Veritrail reads branch protection and required-review settings per repository via the GitHub API and flags noncompliant repos.",
  );
}
...
function gitlab(checkId: string, _s: RemediationSummary): CheckComplianceCopy {
  if (checkId === "gitlab.repo.no_branch_protection") {
    return copy(
      "Require pull requests, review approval, and status checks before changes can merge into the default branch.",
      "Veritrail reads protected-branch and merge-request approval rules on the default branch via the GitLab API. Projects without required reviews or status checks are flagged.",
    );
  }
  if (checkId === "gitlab.repo.no_env_protection") {
    return copy(
      "Require reviewers for production-like deployment environments. Non-production environments should not be treated as High unless explicitly configured that way.",
      "Veritrail reads GitLab protected environment rules. Production-like environments without required approvers are flagged; non-production environments are reported at Medium severity.",
    );
  }
  if (checkId.startsWith("gitlab.org.")) {
    return copy(
      "Verify that GitLab group security settings meet your identity baseline.",
      "Veritrail syncs GitLab group security settings via the GitLab API and flags values outside the configured baseline.",
    );
  }
  return copy(
    "Verify that GitLab protected branches and merge request approval settings meet your change-management baseline.",
    "Veritrail reads protected-branch and merge-request approval rules per project via the GitLab API and flags noncompliant projects.",
  );
}
...
function defaultCopy(checkId: string, s: RemediationSummary): CheckComplianceCopy {
  ...
  return copy(
    ...
    `Veritrail evaluates ${topic} configuration on each scan and flags resources that match this check's failure criteria.`,
  );
}
...
const SPECIFIC: Record<string, (s: RemediationSummary) => CheckComplianceCopy> = {
  "iam.perm.granted_vs_used": () =>
    copy(
      ...
      "Veritrail compares each role's granted write actions against service last-accessed data. Write permissions on services with no recorded use in the lookback window are flagged.",
    ),
  "iam.access_inventory_gap": () =>
    copy(
      ...
      "Veritrail records a scan failure when the IAM user inventory collector cannot complete. No per-user findings are produced until a later scan succeeds.",
    ),
  "iam.account.no_support_role": () =>
    copy(
      "Verify that a dedicated IAM role exists for AWS Support access instead of using root.",
      "Veritrail lists IAM roles and flags the account when no role matches the configured support-access pattern.",
    ),
  "iam.account.password_policy_weak": () =>
    copy(
      ...
      "Veritrail reads the account password policy and compares length, complexity, reuse, and expiration settings to configured thresholds.",
    ),
  "aws.account.contact_incomplete": () =>
    copy(
      "Verify that AWS account primary contact information is complete (address, city, country, phone).",
      "Veritrail reads account primary contact fields via the scan role. Missing address, city, country, or phone is flagged.",
    ),
  "aws.account.security_contact_missing": () =>
    copy(
      ...
      "Veritrail reads alternate contact data for the security alternate contact type. Missing email or phone on that contact is flagged.",
    ),
  "iam.server_certificate.expired": () =>
    copy(
      "Verify that expired IAM server certificates are removed from the account.",
      "Veritrail lists IAM server certificates via ListServerCertificates. Certificates past their expiration date are flagged.",
    ),
  "iam.cloudshell_full_access_granted": () =>
    copy(
      "Verify that AWSCloudShellFullAccess is not attached to non-break-glass IAM principals.",
      "Veritrail lists IAM policy attachments. Principals with AWSCloudShellFullAccess attached are flagged.",
    ),
  "kms.key.policy_wildcard_principal": () =>
    copy(
      "Verify that KMS key policies do not allow wildcard (\"*\") principals.",
      "Veritrail parses KMS key policies. Policies allowing Principal \"*\" are flagged.",
    ),
  "kms.key.no_rotation": () =>
    copy(
      "Verify that customer-managed symmetric KMS keys have automatic annual rotation enabled.",
      "Veritrail reads rotation status on customer-managed symmetric keys. Keys without automatic rotation enabled are flagged.",
    ),
  "guardduty.open_findings": () =>
    copy(
      "Verify that active GuardDuty findings are triaged or remediated.",
      "Veritrail lists active GuardDuty findings via the GuardDuty API. Findings not archived or suppressed are surfaced.",
    ),
  "guardduty.detector.not_enabled": () =>
    copy(
      "Verify that GuardDuty detectors are enabled in each in-scope region.",
      "Veritrail reads GuardDuty detector status per region. Disabled or missing detectors are flagged.",
    ),
  "aws.config.rules_non_compliant": () =>
    copy(
      ...
      "Veritrail lists AWS Config rule evaluation results. Rules reporting non-compliant resources are flagged.",
    ),
  "aws.config.not_enabled": () =>
    copy(
      "Verify that AWS Config recorder and delivery channel are active.",
      "Veritrail reads Config recorder and delivery channel status. Accounts without an active recorder are flagged.",
    ),
  "aws.access_analyzer.not_enabled": () =>
    copy(
      "Verify that IAM Access Analyzer is enabled in active regions.",
      "Veritrail lists Access Analyzer instances per scanned region. Regions without an active analyzer are flagged.",
    ),
  "aws.securityhub.not_enabled": () =>
    copy(
      "Verify that AWS Security Hub is enabled with your organization security standard.",
      "Veritrail reads Security Hub enrollment status. Accounts or regions without Security Hub enabled are flagged.",
    ),
  "vpc.flow_logs.not_enabled": () =>
    copy(
      "Verify that VPC flow logs are enabled and delivering to CloudWatch Logs or S3.",
      "Veritrail reads VPC flow log configuration. VPCs without flow logging enabled are flagged.",
    ),
  "ec2.ami.aged": () =>
    copy(
      "Verify that EC2 workloads launch from AMIs within your patch-age threshold.",
      "Veritrail reads AMI creation dates. AMIs older than the configured patch-age threshold are flagged.",
    ),
  "ec2.ami.public": () =>
    copy(
      "Verify that custom AMIs are not shared publicly (allAccounts launch permission).",
      "Veritrail reads AMI launch permissions. AMIs with allAccounts (public) launch permission are flagged.",
    ),
  "ec2.instance.imdsv2_not_required": () =>
    copy(
      "Verify that EC2 instances require IMDSv2 (HttpTokens required).",
      "Veritrail reads instance metadata options. Instances without HttpTokens set to required are flagged.",
    ),
  "ec2.ebs.encryption_not_default": () =>
    copy(
      "Verify that EBS encryption-by-default is enabled in each region.",
      "Veritrail reads EBS encryption-by-default settings per region. Regions where default encryption is disabled are flagged.",
    ),
  "ec2.ebs.volume_unencrypted": () =>
    copy(
      "Verify that attached EBS volumes are encrypted at rest.",
      "Veritrail lists attached EBS volumes. Volumes with encryption disabled are flagged.",
    ),
  "ec2.ebs.snapshot_public": () =>
    copy(
      "Verify that EBS snapshots are not shared publicly.",
      "Veritrail reads snapshot create-volume permissions. Snapshots shared with all AWS accounts are flagged.",
    ),
  "ec2.ebs.snapshot_unencrypted": () =>
    copy(
      "Verify that EBS snapshots are encrypted.",
      "Veritrail lists EBS snapshots. Snapshots without encryption are flagged.",
    ),
  "rds.instance.publicly_accessible": () =>
    copy(
      "Verify that RDS instances are not publicly accessible.",
      "Veritrail reads RDS instance PubliclyAccessible attribute. Instances set to publicly accessible are flagged.",
    ),
  "rds.instance.no_encryption": () =>
    copy(
      "Verify that RDS instances have storage encryption at rest enabled.",
      "Veritrail reads RDS storage encryption settings. Instances without storage encryption are flagged.",
    ),
  "rds.instance.no_automated_backup": () =>
    copy(
      "Verify that RDS automated backups are enabled with retention meeting your policy minimum.",
      "Veritrail reads automated backup and retention settings. Instances with backups disabled or zero retention are flagged.",
    ),
  "rds.instance.no_deletion_protection": () =>
    copy(
      "Verify that production RDS instances have deletion protection enabled.",
      "Veritrail reads RDS deletion protection. Instances without deletion protection enabled are flagged.",
    ),
  "rds.instance.no_multi_az": () =>
    copy(
      "Verify that production RDS instances use Multi-AZ for high availability.",
      "Veritrail reads RDS Multi-AZ configuration. Single-AZ instances are flagged when this check is in scope.",
    ),
  "dynamodb.table.no_encryption": () =>
    copy(
      "Verify that DynamoDB tables have encryption at rest enabled.",
      "Veritrail reads table SSE configuration. Tables without server-side encryption at rest are flagged.",
    ),
  "dynamodb.table.no_pitr": () =>
    copy(
      "Verify that DynamoDB tables have point-in-time recovery enabled.",
      "Veritrail reads continuous backup (PITR) settings. Tables without PITR enabled are flagged.",
    ),
  "acm.certificate.expiring": () =>
    copy(
      "Verify that ACM certificates are renewed before expiry within the warning window.",
      "Veritrail reads ACM certificate NotAfter dates. Certificates expiring within the configured warning window are flagged.",
    ),
  "lambda.function.deprecated_runtime": () =>
    copy(
      "Verify that Lambda functions run on supported runtimes.",
      "Veritrail reads function runtime identifiers. Functions on deprecated or unsupported runtimes are flagged.",
    ),
  "lambda.function.no_dlq": () =>
    copy(
      "Verify that asynchronously invoked Lambda functions have a dead-letter queue configured.",
      "Veritrail reads async-invoke DLQ configuration. Asynchronous functions without a configured dead-letter target are flagged.",
    ),
  "rds.snapshot.public": () =>
    copy(
      "Verify that RDS manual snapshots are not publicly restorable.",
      "Veritrail reads DB snapshot restore attributes. Snapshots shared with all AWS accounts are flagged.",
    ),
  "eks.cluster.public_endpoint": () =>
    copy(
      "Verify that EKS cluster API endpoints are not public to the internet.",
      "Veritrail reads EKS endpoint access configuration and public CIDR ranges. Public endpoints open to 0.0.0.0/0 or ::/0 are flagged.",
    ),
  "ecs.cluster.container_insights_disabled": () =>
    copy(
      ...
      "Veritrail reads ECS cluster settings via the AWS API. Clusters without Container Insights enabled are flagged.",
    ),
  "ecs.service.public_ip_enabled": () =>
    copy(
      "Verify that ECS services do not assign public IPs to tasks in awsvpc mode.",
      "Veritrail reads ECS service network configuration. Services with assignPublicIp ENABLED are flagged.",
    ),
  "ecs.task_definition.privileged_container": () =>
    copy(
      "Verify that active ECS task definitions do not run privileged containers.",
      "Veritrail reads in-use task definitions for running services. Task definitions with privileged containers are flagged.",
    ),
  "lambda.function.public_url": () =>
    copy(
      "Verify that Lambda function URLs are not publicly invokable without authentication.",
      "Veritrail reads Lambda function URL auth configuration. Function URLs with AuthType NONE are flagged.",
    ),
  "ecr.repository.image_scan_disabled": () =>
    copy(
      "Verify that ECR repositories scan container images when images are pushed.",
      "Veritrail reads ECR image scanning configuration. Repositories without scan-on-push enabled are flagged.",
    ),
  "aws.vulnerability_monitoring.not_detected": () =>
    copy(
      ...
      "Veritrail checks for ECR repositories, ECS services, or EKS clusters first. When container workloads exist, it looks for Inspector ECR coverage, ECR enhanced scanning, or scan-on-push. Accounts with container evidence but no scanning source are flagged.",
    ),
  "secretsmanager.secret.no_rotation": () =>
    copy(
      "Verify that Secrets Manager secrets have automatic rotation enabled.",
      "Veritrail reads Secrets Manager rotation configuration. Secrets without automatic rotation enabled are flagged.",
    ),
  "ssm.parameter.plaintext_secret": () =>
    copy(
      ...
      "Veritrail lists SSM String parameters and flags names matching likely-secret patterns stored as plaintext String type.",
    ),
  "elb.load_balancer.no_access_logs": () =>
    copy(
      "Verify that load balancers have access logging enabled to S3.",
      "Veritrail reads load balancer access log configuration. Load balancers without access logging enabled are flagged.",
    ),
  "elb.load_balancer.weak_tls_policy": () =>
    copy(
      ...
      "Veritrail reads listener TLS security policies. Policies below the configured TLS baseline are flagged.",
    ),
  "sns.topic.no_encryption": () =>
    copy(
      "Verify that SNS topics use SSE-KMS encryption at rest.",
      "Veritrail reads SNS topic encryption settings. Topics without SSE-KMS encryption are flagged.",
    ),
  "sqs.queue.no_encryption": () =>
    copy(
      "Verify that SQS queues use SSE-KMS encryption at rest.",
      "Veritrail reads SQS server-side encryption settings. Queues without KMS encryption are flagged.",
    ),
};
