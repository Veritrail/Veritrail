...
const rows: ReferenceRow[] = [
  ...
  { key: "rds.snapshot", value: "RDS manual snapshots and public restore permissions.", examples: ["rds.snapshot.public"], group: "RDS" },
  { key: "eks.cluster", value: "EKS clusters, API endpoint access mode, public CIDR ranges, version, and status.", examples: ["eks.cluster.public_endpoint"], group: "EKS" },
  { key: "ecs.cluster", value: "ECS clusters and Container Insights setting.", examples: ["ecs.cluster.container_insights_disabled"], group: "ECS" },
  { key: "ecs.service", value: "ECS services, launch type, and awsvpc public IP assignment.", examples: ["ecs.service.public_ip_enabled"], group: "ECS" },
  { key: "ecs.task_definition", value: "In-use ECS task definitions referenced by running services.", examples: ["ecs.task_definition.privileged_container"], group: "ECS" },
  { key: "lambda.function", value: "Lambda functions, runtime version, function URL auth, and dead-letter queue configuration.", examples: ["lambda.function.deprecated_runtime", "lambda.function.public_url", "lambda.function.no_dlq"], group: "Lambda" },
  { key: "ecr.repository", value: "ECR repositories, image scan-on-push state, and encryption type.", examples: ["ecr.repository.image_scan_disabled"], group: "ECR" },
